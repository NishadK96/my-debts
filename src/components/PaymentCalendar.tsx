    ...
    <section className="panel p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Upcoming payments grouped by due date.</p>
        </div>
        <CalendarDays className="text-blue-600" size={22} />
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {groups.map((group) => {
          const dueState = getDueState(group.date);
          const paid = group.payments.every((payment) => payment.paid);

          return (
            <article key={group.date.toISOString()} className={`rounded-lg border p-4 ${dueState === 'overdue' ? 'border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/30' : dueState === 'soon' ? 'border-orange-200 bg-orange-50 dark:border-orange-900 dark:bg-orange-950/30' : 'border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950'}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">{formatDate(group.date)}</p>
